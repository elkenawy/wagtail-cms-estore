...
class TestElasticsearch2SearchQuery(TestCase):
    def assertDictEqual(self, a, b):
        ...
        self.assertEqual(
            json.dumps(a, sort_keys=True, default=default), json.dumps(
                b, sort_keys=True, default=default)
        )
    ...
    def test_and_filter(self):
        # Create a query
        query_compiler = self.query_compiler_class(models.Book.objects.filter(
            title="Test", publication_date=datetime.date(2017, 10, 18)), "Hello")

        # Check it
        expected_result = {'filtered': {'filter': {'and': [
            {'match': {'content_type': 'searchtests.Book'}},
            {'and': [{'term': {'publication_date_filter': '2017-10-18'}},
                     {'term': {'title_filter': 'Test'}}]}
        ]}, 'query': {'multi_match': {'query': 'Hello', 'fields': ['_all', '_partials']}}}}
    ...
    def test_or_filter(self):
        # Create a query
        query_compiler = self.query_compiler_class(models.Book.objects.filter(
            Q(title="Test") | Q(publication_date=datetime.date(2017, 10, 18))), "Hello")
        ...
        expected_result = {'filtered': {'filter': {'and': [
            {'match': {'content_type': 'searchtests.Book'}},
            {'or': [{'term': {'publication_date_filter': '2017-10-18'}},
                    {'term': {'title_filter': 'Test'}}]}
        ]}, 'query': {'multi_match': {'query': 'Hello', 'fields': ['_all', '_partials']}}}}
        self.assertDictEqual(query, expected_result)
    ...
    def test_negated_filter(self):
        # Create a query
        query_compiler = self.query_compiler_class(models.Book.objects.exclude(
            publication_date=datetime.date(2017, 10, 18)), "Hello")
    ...
    def test_gt_lookup(self):
        ...
        query_compiler = self.query_compiler_class(
            models.Book.objects.filter(
                publication_date__gt=datetime.datetime(2014, 4, 29)), "Hello"
        )
    ...
    def test_lt_lookup(self):
        ...
        query_compiler = self.query_compiler_class(
            models.Book.objects.filter(
                publication_date__lt=datetime.datetime(2014, 4, 29)), "Hello"
        )
    ...
    def test_gte_lookup(self):
        ...
        query_compiler = self.query_compiler_class(
            models.Book.objects.filter(
                publication_date__gte=datetime.datetime(2014, 4, 29)), "Hello"
        )
    ...
    def test_lte_lookup(self):
        ...
        query_compiler = self.query_compiler_class(
            models.Book.objects.filter(
                publication_date__lte=datetime.datetime(2014, 4, 29)), "Hello"
        )
    ...
    def test_range_lookup(self):
        ...
        query_compiler = self.query_compiler_class(
            models.Book.objects.filter(
                publication_date__range=(start_date, end_date)), "Hello"
        )

        # Check it
        expected_result = {'filtered': {'filter': {'and': [
            {'match': {'content_type': 'searchtests.Book'}},
            {'range': {'publication_date_filter': {
                'gte': '2014-04-29', 'lte': '2014-08-19'}}}
        ]}, 'query': {'multi_match': {'query': 'Hello', 'fields': ['_all', '_partials']}}}}
        self.assertDictEqual(query_compiler.get_query(), expected_result)
    ...
    def test_phrase_query(self):
        ...
        # Check it
        expected_result = {'multi_match': {'fields': [
            '_all', '_partials'], 'query': "Hello world", 'type': 'phrase'}}
...
class TestElasticsearch2Mapping(TestCase):
    ...
    def setUp(self):
        # Create ES mapping
        self.es_mapping = Elasticsearch2SearchBackend.mapping_class(
            models.Book)
    ...
    def test_get_document_id(self):
        self.assertEqual(self.es_mapping.get_document_id(
            self.obj), 'searchtests_book:' + str(self.obj.pk))
...
class TestElasticsearch2MappingInheritance(TestCase):
    ...
    def test_get_document(self):
        ...
        if 'searchtests_novel__characters' in document:
            document['searchtests_novel__characters'].sort(
                key=lambda c: c['name'])
